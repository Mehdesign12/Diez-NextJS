...
export async function generateStaticParams() {
  const combos = await getAllPseoCityServiceParams();
  return SUPPORTED_LANGS.flatMap((lang) =>
    combos.map((c) => ({ lang, city: c.city, service: c.service }))
  );
}
...
export async function generateMetadata({
  ...
  const { lang, city: citySlug, service: serviceSlug } = await params;
  ...
  const page = await getPseoPageByCityAndService(city.id, serviceSlug);
  ...
  const url = `https://diez-agency.com/${lang}/agence/${citySlug}/${serviceSlug}`;
  ...
  return {
    ...
    alternates: {
      canonical: url,
      languages: {
        fr: `https://diez-agency.com/fr/agence/${citySlug}/${serviceSlug}`,
        en: `https://diez-agency.com/en/agence/${citySlug}/${serviceSlug}`,
        'x-default': `https://diez-agency.com/en/agence/${citySlug}/${serviceSlug}`,
      },
    },
    openGraph: {
      type: 'website',
...
export default async function CityServicePage({
  ...
  const serviceJsonLd = {
    ...
    url: `https://diez-agency.com/${lang}/agence/${citySlug}/${serviceSlug}`,
    provider: {
      '@type': 'Organization',
      name: 'Diez Agency',
  ...
  return (
    <>
      ...
      <Navbar />
      <PseoPage city={city} page={page} otherCities={otherCities} serviceSlug={serviceSlug} />
      <Footer />
    </>
  );
}
